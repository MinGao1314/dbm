...
def beta_Hermite_Gaussian_ensemble(dim, beta):
    diag = np.sqrt(2)*np.diag(np.random.randn(dim))
    u_diag_vect = np.zeros(dim-1)
    for i in range(0,dim-1):
        u_diag_vect[i] = np.sqrt(np.random.chisquare((dim-(i+1))*beta/4))
...
class tridiag_dyson:
    def __init__(self, n_traj, n_samples, tf):
        ...
        init_mat = beta_Hermite_Gaussian_ensemble(self.dim, beta=2)
        ...
        if l <= k:
            ...
            for eigenvalue in eigenvalues:
                if eigenvalue != lbda :
                    ...
                    p_kl += self.spectral_weight(eigenvalue)**2 * P.polyval(eigenvalue, self.p[k]) * frac
                else:
                    deriv = P.polyval(lbda, P.polyder(self.p[l]))
                    p_kl += self.spectral_weight(eigenvalue)**2 * P.polyval(eigenvalue, self.p[k]) * deriv
        return p_kl
    ...
    def compute_G_kl(self, k, l, U, eigenvalues):
        ...
        if k == l:
            ...
            for eigenvalue in eigenvalues:
                ...
                sum += self.spectral_weight(eigenvalue)**2 * P.polyval(eigenvalue, self.p[k]) * acc
            G_kl[0,0] = 0.5 * sum
    ...
    def compute_E(self, lbda, i):
        delta_ii = np.zeros((self.dim, self.dim))
        delta_ii[i, i] = 1
        O = np.zeros((self.dim, self.dim))
        for k in range(self.dim):
            q_k = self.spectral_weight(lbda[k])
            for l in range(self.dim):
                O[l, k] = q_k * P.polyval(lbda[k], self.p[l])
        q_i = self.spectral_weight(lbda[i])
        E = np.dot(np.dot(O.T, (1 / q_i ** 2) * delta_ii), O)
        self.O = O
        self.E = E
        return E

    def compute_brownians(self):
        draw_brownians = self.brownians[-1] + np.random.randn(self.n_traj) * self.dt ** 0.5
    ...
    def generate(self):
        for sample in range(1, self.n_samples+1):
            ...
            for i in range(self.n_traj):
                ...
                d_A -= (self.brownians[sample][i] + self.dt*(-2*lbda_i + sum_term))*(self.spectral_weight(lbda_i)**2)*G_i
            #print(d_A)
            ...
            for i in range(0, self.dim):
                ...
                for k in range(0, self.dim):
                    for l in range(0, self.dim):
                        ...
                        d_P -= 0.5 * self.spectral_weight(lbda_i)**2 * add * self.compute_G_kl(k, l, self.tridiag_matrices[sample-1], self.eigen_values[sample-1]) * self.dt
